...
export function generate_sst_conditions(
  ...
  const goLabels = condition_labels.filter((label) => label.startsWith("go"));
  const stopLabels = condition_labels.filter((label) => label.startsWith("stop"));
  const nStop = Math.round(n_trials * stop_ratio);
  const nGo = n_trials - nStop;

  const counts = new Map<string, number>();
  const baseGo = Math.floor(nGo / goLabels.length);
  const remGo = nGo % goLabels.length;
  goLabels.forEach((label, index) => counts.set(label, baseGo + (index < remGo ? 1 : 0)));

  const baseStop = Math.floor(nStop / stopLabels.length);
  const remStop = nStop % stopLabels.length;
  stopLabels.forEach((label, index) => counts.set(label, baseStop + (index < remStop ? 1 : 0)));
